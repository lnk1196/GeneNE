...
try:
        ...
        html.H6('Created by Lauren Kirsch and Dr. Chiquito Crasto'),
            html.Label(' Search a Human Gene  '),
        ...
        html.Br(),
        html.Div([
            dcc.Input(id="search_PubMed", 
                  type="text",
                  value = '',
                  placeholder="PubMed ID Lookup ...",
                  debounce = True,
                  minLength = 0, maxLength = 50,
                  autoComplete='on',
                  size='40')
        ...
        def update_output(search_gene, vertical_display_user_slct, synonym_display_user_slct):
            df = pd.read_csv('list_out_df.txt', sep = '\t', dtype=str)
        ...
        @app.callback(Output('outlink', 'children'),
                  Input('search_PubMed', 'value'))
        def links(search_PubMed):
            if len(search_PubMed) != 0:
                return (html.A('Click for PubMed Links', href='/assets/'+search_PubMed+'.html', target="_blank"))
